...
class Sampler_Classifer_Network_Pretrain(LightningModule):
    def __init__(self, sampler, classifier, loop_parameter, classifier_start, mask_per, save_path):
        ...
        wandb.init("sampler_classifier_pretrain")
    ...
    def training_step(self, batch, batch_idx):
        ...
        return {"Accuracy":accuracy, "Loss":loss, "LR": torch.tensor(classifier_scheduler.get_last_lr())}
    ...
    def configure_optimizers(self):
        ...
        classifier_scheduler=torch.optim.lr_scheduler.ExponentialLR(classifier_optimizer,1)# #classifier_scheduler = torch.optim.lr_scheduler.OneCycleLR(
        #                    #     classifier_optimizer,
        #                     ###  steps_per_epoch=782)
        sampler_scheduler =torch.optim.lr_scheduler.ExponentialLR(sampler_optimizer,1) # #sampler_scheduler    = torch.optim.lr_scheduler.OneCycleLR(
        #                         sampler_optimizer,
        #                         0.1,
